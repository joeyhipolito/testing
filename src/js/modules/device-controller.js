...
((Liveswitch, $) => {
  const DeviceController = (() => {
    function DeviceController(localMedia) {
      this.hidden = true;
      ...
      const container = $('#ls-device-container');
      // add close button with font awesome
      container.append(`
      <div class="absolute top-2 right-4">
        <button class="text-white text-2xl font-bold" id="ls-device-close">
          <i class="fa fa-times"></i>
        </button>
      </div>
      `).on('click', '#ls-device-close', () => {
        this.toggle();
      }).addClass('pt-8')

    }
    DeviceController.getInstance = function (localMedia) {
      if (DeviceController.instance == null) {
        DeviceController.instance = new DeviceController(localMedia);
      }
      return DeviceController.instance;
    };
    DeviceController.prototype.toggle = function () {
      const lsChatContainer = $('#ls-device-container');
      if (this.hidden) {
    ...
    DeviceController.prototype.onMicrophoneDeviceChange = function (id) {
      const deviceId = id;
    ...
    DeviceController.prototype.onVideoDeviceChange = function (id) {
      const deviceId = id;
    ...
    DeviceController.prototype.renderMicrophoneSelect = function () {
      const container = $('#ls-device-container');
      container.append(`
    ...
    DeviceController.prototype.renderVideoSelect = function () {
      const container = $('#ls-device-container');
      container.append(`
      <div class="w-full mx-auto mb-4">
  ...
  Liveswitch.DeviceController = DeviceController;
})(Liveswitch, $);